...
def marker():
    ...
    for i in range(trial_times-1):  #not contain last trial because of the lose of the falling edge of push_onoff
        ...
        while event_series[i,0]<pert_on[n] and pert_on[n]<event_series[i+1,0] and n<len(pert_on)-2:
    ...
    print(event_series[0:150])
    pd.DataFrame(event_series).to_csv('C:/Users/zyh20/Desktop/event_series.csv')
...
def Pattern_Entropy_eachtrial(data): 
    ...
    for j in range(0,len(data)):
        ...
        print(len(trial))
        for i in range(0,len(trial)-len(trial)%8,8):  # delete end bits that can't be divide by 8
            a = np.array(trial[i+5:i+13])                # slice into 8 bit,  1 byte(1字节)(1B) = 8 bit(8比特)(8位二进制)；1KB = 1024B; 1MB = 1024KB; 1GB = 1024MB
            #statistic double spike
            if len(a)==8:
                if double_spike_time(a)==True:
                    trial_num=np.append(trial_num,j)
                    time_num=np.append(time_num,i)
                    print(a)
    print(trial_num)
    print(time_num)
...
def pattern_extract():
    ...
    for i in event_series:  
        if i[4]!=0:
            ...
            pert=np.append(pert,i[5])
    
    for j in pert:
        print(j)
    binar_pert=binary_spiketrain(348,pert,0,0.2)
    
    trial_num=Pattern_Entropy_eachtrial(binar_pert)
...
def push_pattern_extract():
    ...
    event_series=marker()
    for i in range():
        before_push=event_series[i,0]

    binar_pert=binary_spiketrain(348,before_push,0,0.2)
    
    Pattern_Entropy_eachtrial(binar_pert)

def double_spike_time(str):
    for i in range(0,7):
